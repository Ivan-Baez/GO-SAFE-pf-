"use client";
import { Field, useFormikContext } from "formik";

interface StepProps {
  prev: () => void;
}

export default function EducationStep({ prev }: StepProps) {

  const { values } = useFormikContext<any>();

  return (
    <div className="flex flex-col items-center w-full max-w-md mx-auto p-6 min-h-screen bg-white">
      
      <div className="w-full text-left mb-6">
        <h2 className="text-2xl font-bold text-[#1e3c31] mb-2">Educación</h2>
        <p className="text-gray-600 text-sm">
          Cuéntanos sobre tu formación académica o profesional.
        </p>
      </div>

      <div className="w-full space-y-4 z-10">
        
        {/* Título */}
        <div>
          <label className="block font-bold text-[#1e3c31] mb-2">
            Título / Carrera
          </label>
          <Field 
            name="career" 
            placeholder="Ej: Lic. en Deporte" 
            className="inputStyles w-full"
          />
        </div>

        {/* Institución */}
        <div>
          <label className="block font-bold text-[#1e3c31] mb-2">
            Institución
          </label>
          <Field 
            name="institution" 
            placeholder="Nombre de la universidad o escuela" 
            className="inputStyles w-full"
          />
        </div>

        {/* Nivel */}
        <div>
          <label className="block font-bold text-[#1e3c31] mb-2">
            Nivel
          </label>
          <Field
            as="select"
            name="level"
            className="inputStyles w-full appearance-none italic text-gray-400"
          >
            <option value="" disabled>Selecciona nivel</option>
            <option value="universitario">Universitario</option>
            <option value="terciario">Terciario</option>
            <option value="curso">Curso / Certificación</option>
            <option value="secundario">Secundario</option>
          </Field>
        </div>

        {/* Años */}
        <div className="flex gap-4">
          <div className="w-1/2">
            <label className="block font-bold text-[#1e3c31] mb-2 text-sm">
              Año Inicio
            </label>
            <Field
              name="añoInicio"
              type="number"
              placeholder="2020"
              className="inputStyles w-full"
            />
          </div>

          <div className="w-1/2">
            <label className="block font-bold text-[#1e3c31] mb-2 text-sm">
              Año Fin
            </label>
            <Field
              name="añoFin"
              type="number"
              placeholder="2024"
              className={`inputStyles w-full ${
                values.actualidad ? "opacity-30 pointer-events-none" : ""
              }`}
            />
          </div>
        </div>

        {/* Checkbox */}
        <div className="flex items-center space-x-3 pt-2">
          <Field
            type="checkbox"
            name="onCourse"
            id="onCourse"
            className="w-5 h-5 border-gray-300 rounded text-[#f0ba3c] focus:ring-[#f0ba3c]"
          />
          <label htmlFor="onCourse" className="text-gray-600 text-sm">
            Actualmente estudiando
          </label>
        </div>

        {/* Botones */}
        <div className="flex items-center justify-between gap-4 pt-6">

          <button
            type="button"
            onClick={prev}
            className="text-gray-400 text-sm font-medium hover:underline mt-3"
          >
            Volver al paso anterior
          </button>

          <button
            type="submit"
            className="w-full min-w-[150px] max-w-[280px] bg-[#f0ba3c] hover:bg-[#e0ab2c] text-white font-bold py-3 px-6 rounded-2xl shadow-lg transition-all duration-300"
          >
            Continuar
          </button>

        </div>
      </div>
    </div>
  );
}